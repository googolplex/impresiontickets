...
*  MODIFICADO POR ROGER ARMOA para IMPRESION DE TICKETS 15.10.2018
...
*  DESPLIEGA ESTADO DE CTA. DEL CLIENTE
   PROCEDURE DESPESTA
      ...
      @20,00 say replicate("�",78)
      set colo to w+/b
      @21,03 say " <F4>Cobrar  <F5>Act. Cobros  <F6>Reclamos  <F7>Mensajes  <Esc>Cancelar "
      set colo to i
      modi comm C:\-DATOS\SICMADAT\CCE\&v_arcaux noedit window interior
      dele file C:\-DATOS\SICMADAT\CCE\&v_arcaux
      deac wind estadcta
...
* IMPRESION DE TICKETS 2018.10.15a
   PROCEDURE IMPCABFAC
	...
	If v_fecalt <> ctod("//")
		...
		@ prow()   , 83  say year(date()) pict [9999]
	Else
		If v_feccob <> date()
			@ prow()+6 , 34  say day(v_feccob) pict [99]
			@ prow()   , 60  say v_imes pict [@!]
			@ prow()   , 83  say year(v_feccob) pict [9999]
		Else
			@ prow()+6 , 34  say day(date()) pict [99]
			@ prow()   , 60  say v_imes pict [@!]
			@ prow()   , 83  say year(date()) pict [9999]
