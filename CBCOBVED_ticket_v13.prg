...
*  TICKETFELIZ 2018.10.15
   ...
   *@ 1,1 say "impresion con arrobas"
      ...
      *@ prow()+4, 00  say [ ]
      ...
      if v_totdescue < 0
         v_linea = v_linea  + 1
	* traigo a la izquierda 2018.10.19a
         @ prow()+1,0  say "Descuento..."
         @ prow(), 20 say v_totdescue pict  "999,999,999"
      endif

      * ya no genero lineas en blanco
      * if v_linea < 22
      *   for i = v_linea to 22
      *       @ prow()+1,00 say [ ]
      *   endfor
...
* MODIFICADO el 2018.10.18a
      ...
      @ prow()+1,0 say [TOTAL A PAGAR: ]
      @ prow(),30 say v_totfac  pict "999,999,999"
      @ prow()+1, 0 say subs(valnel,1,60) &&( Importe en Letras )
      @ prow()+1,0 say [IVA05: ]
      @ prow()+1,0 say [IVA10: ]
      @ prow(),30 say v_valiva pict "99,999,999"
      @ prow()+1,0 say [TOTAL IVA: ]
      @ prow(),30 say v_valiva pict "99,999,999"
...
* IMPRESION DE TICKETS 2018.10.15a
   PROCEDURE IMPCABFAC
      ...
      * @ prow() + v_linea,5 say [PARQUE SERENIDAD S.R.L.]
      ...
      @ prow() + 1,0 say "FACTURA: 006-001-"
      @ prow() , 15  say trim(v_tipfac)
      @ prow() , 30  say v_nrofac pict [99999999999999]
      @ prow() + 1,0 say "VALIDO DESDE: 18/12/2017"
      @ prow() + 1,0 say "VALIDO HASTA: 31/12/2017"
      @ prow() + 1,0 say "CANTIDAD DESCRIPCION PU PRECIO IVA"
      @ prow() + 1,0  say [FECHA ]
	If v_fecalt <> ctod("//")
		@ prow() , 10  say day(date()) pict [99]
		@ prow() , 13  say v_imes pict [@!]
		@ prow() , 25  say year(date()) pict [9999]
	Else
		If v_feccob <> date()
			@ prow() , 10  say day(v_feccob) pict [99]
			@ prow() , 13  say v_imes pict [@!]
			@ prow() , 25  say year(v_feccob) pict [9999]
		Else
			@ prow() , 10  say day(date()) pict [99]
			@ prow() , 13  say v_imes pict [@!]
			@ prow() , 25  say year(date()) pict [9999]
		Endif
	Endif
* comentado por roger el 18.10.2018
*  	  @ prow()   , 135 say "X"

      v_linea  = v_linea  + 1
      @ prow() + 1,0  say [CLIENTE ]
      @ prow(), 15  say trim(v_ructit)
      @ prow()+1, 0  say trim(v_nomtit)+[ - ]+[(]+tran(v_nrocta,"999,999")+[)]
      v_linea = v_linea + 1
*      @ prow()+4, 21  say [ ]
   RETURN
...
************   Fin de eleccion del ultimo mes  *************
      ...
      do case
         case saldoact(k,07) = "SEM"
            v_detalle = "Sem " + saldoact(k,01) + transform(saldoact(k,02),[999999])  +;
                        ", de " + v_cuomen + "/" + transform(v_anomen,[9999]) + " a " + v_cuomay +;
                        "/" + transform(v_anomay,[9999])
         case saldoact(k,07) = "ANU"
            v_detalle = "Anual " + saldoact(k,01) + transform(saldoact(k,02),[999999])      +;
                        ", de " + v_cuomen + "/" + transform(v_anomen,[9999]) + " a " + v_cuomay +;
                        "/" + transform(v_anomay,[9999])
      endcase
      do imprideta
...
* LISTA DE LLAMADAS POR CLIENTES.
  ...
  ******************************************************
  ...
  set printer to M:\temp\&v_disacco
  set device  to printer
  ...
  *set printer to prn
  set console on
  modify comm M:\temp\&v_disacco noedit window acticobro
  delete file M:\temp\&v_disacco
  set color to i
...
PROCEDURE AVISOCOBRA
  ...
  *delete file M:\-DATOS\SICMADAT\CCE\&v_disacco
  modify comm M:\temp\&v_disacco noedit window acticobro
  delete file M:\temp\CCE\&v_disacco
